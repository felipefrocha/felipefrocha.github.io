...
build:
	...
	@echo "  1. Generate static JSON content"
	@echo "  2. Build React frontend to $(BUILD_DIR)"
	@echo ""
...
verify: build
	...
	@if [ ! -d "src/apps/client/public/api" ]; then \
		echo "$(RED)✗ public/api directory not found (missing static JSON API)$(NC)"; \
		exit 1; \
	fi
